...
func payloadFunc(data interface{}) jwt.MapClaims {
	if v, ok := data.(map[string]interface{}); ok {
		...
		// 将用户json转为结构体
		common.JsonI2Struct(v["user"], &user)
		return jwt.MapClaims{
			jwt.IdentityKey: user.UserId,
			"user":          v["user"],
		}
	}
	return jwt.MapClaims{}
...
func login(c *gin.Context) (interface{}, error) {
	...
	//通过token去拿用户信息
	info, err := cache.GetUserOnlineInfo(token)
	if err != nil {
		return "", errors.New(fmt.Sprintf("Access Denied"))
	}
	//用户未登陆
	if !info.OnLine {
		c.Abort()
		return "", errors.New(fmt.Sprintf("No Login"))
	}
	// 将用户以json格式写入, payloadFunc/authorizator会使用到
	return map[string]interface{}{
		"user": common.Struct2Json(info),
	}, nil
}
...
func authorizator(data interface{}, c *gin.Context) bool {
	if v, ok := data.(map[string]interface{}); ok {
		...
		var user model.UserOnline
		// 将用户json转为结构体
		common.Json2Struct(userStr, &user)
...
func unauthorized(c *gin.Context, code int, message string) {
	...
	//response.Response(c, code, code, nil, fmt.Sprintf("认证失败, 错误码: %d, 错误信息: %s", code, message))
	response.Response(c, code, code, nil, "认证失败, 账号或密码错误")
}
